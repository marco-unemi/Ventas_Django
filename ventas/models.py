...
from project_sales.utils import valida_cedula,phone_regex
...
class Brand(models.Model):
    description = models.CharField('Brand',max_length=100)
...
class Product(models.Model):
    ...
    image = models.ImageField(upload_to='products/',blank=True,null=True,default='products/product.png')
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
    state = models.BooleanField('Activo', default = True)
...
class Company(models.Model):
    ...
    ruc = models.BigIntegerField(unique=True)  # BigIntegerField si el RUC puede ser un número grande

    def __str__(self) -> str:
        return f'Company-{self.id}: {self.business_name}'